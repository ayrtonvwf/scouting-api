...
class Question extends MY_Controller {
	...
	public function get() : void {
		...
		$input_fields = ['id', 'period_id', 'question_type_id', 'search'];
		if ($this->data) {
		...
		$questions = $this->question_model->search($search['id'], $search['period_id'], $search['question_type_id'], $search['search']);
		$questions = array_map(function($question) {
			return get_array_values($question, ['id', 'period_id', 'question_type_id', 'description', 'created_at']);
		}, $questions);
	...
	private function _get_validate() : bool {
		...
		$this->form_validation->set_rules('period_id', 'Period Id', 'strip_tags|trim|integer');
